...
export class FileOpsParser {
  ...
  /**
   * Validate file operation (existence, project boundaries, etc.)
   */
  private static async validateFileOperation(op: FileOp): Promise<FileValidationResult> {
    try {
      ...
      try {
        const content = await agentClient.readFileContent(op.path);
        fileExists = content !== null;
      } catch (error) {
        fileExists = false;
      }
      
      // Basic project path validation - ensure it's a relative path and doesn't escape project
      const isWithinProject = this.isValidProjectPath(op.path);

      if (!isWithinProject) {
        return {
          isValid: false,
          exists: fileExists,
          error: `Path "${op.path}" is outside project boundaries or uses absolute paths`
        };
      }

      // Check for dangerous paths
      const isDangerous = this.isDangerousPath(op.path);
      if (isDangerous) {
        return {
          isValid: false,
          exists: fileExists,
          error: `Path "${op.path}" targets system files or sensitive directories`
        };
      }

      // Action-specific validation
      if (op.action === 'edit' && !fileExists) {
        return {
          isValid: false,
          exists: false,
          error: `Cannot edit non-existent file: ${op.path}`
        };
      }

      if (op.action === 'delete' && !fileExists) {
        return {
          isValid: false,
          exists: false,
          error: `Cannot delete non-existent file: ${op.path}`
        };
      }

      if (op.action === 'create' && fileExists) {
        return {
          isValid: true,
          exists: true,
          warning: `File ${op.path} already exists and will be overwritten`
        };
      }

      // Language compatibility check
      if (op.content) {
        const languageValidation = this.validateFileLanguage(op.path, op.content);
        if (!languageValidation.isValid) {
          return {
            ...
            expectedLanguage: languageValidation.expectedLanguage,
            detectedLanguage: languageValidation.detectedLanguage
          };
        }
        if (languageValidation.warning) {
          return {
            isValid: true,
            exists: fileExists,
            warning: languageValidation.warning,
  ...
  /**
   * Basic validation for project paths
   */
  private static isValidProjectPath(filePath: string): boolean {
  ...
  private static isDangerousPath(filePath: string): boolean {
    ...
    return dangerous.some(danger => lowerPath.includes(danger));
  }

  /**
   * Validate file language compatibility
   */
  private static validateFileLanguage(filePath: string, content: string): {
    isValid: boolean;
    error?: string;
    warning?: string;
    expectedLanguage?: string;
    ...
    try {
      const expectedLanguage = detectLanguageFromPath(filePath);
      const langInfo = getLanguageInfoFromPath(filePath);
      
      if (!langInfo) {
        return { isValid: true }; // Unknown file type, skip validation
      }

      // Use existing language validation logic
      return this.validateContentLanguage(content, expectedLanguage, langInfo.name);
    } catch (error) {
      return {
        isValid: false,
        error: `Language validation error: ${error}`
      };
    }
  }
