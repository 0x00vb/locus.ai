...
contextBridge.exposeInMainWorld('api', {
  ...
  lsp: {
    isLanguageSupported: (language: string) => 
      ipcRenderer.invoke('lsp:isLanguageSupported', language),
    getPort: () => 
      ipcRenderer.invoke('lsp:getPort'),
    onPortAvailable: (callback: (port: number) => void) => {
      ipcRenderer.on('lsp-port', (_: any, port: number) => callback(port));
    },
    removePortListener: () => {
      ipcRenderer.removeAllListeners('lsp-port');
    }
  },
});
